...
export async function Footer({ variant = "landing" }: FooterProps) {
  ...
  return (
    <footer className="border-t border-ash px-6 py-8 md:px-8">
      {variant === "landing" ? (
        <div className="flex items-center justify-between gap-4">
          <LogoMonogram size={28} title="Tears of God" />
          <MetaStrip left={t("Footer.copyright", { year })} right="IG · YT · SPOTIFY · BANDCAMP" />
        </div>
      ) : (
      ...
      <div className="text-meta text-steel-text mt-6 flex gap-4 border-t border-ash pt-4 font-mono tracking-wide uppercase">
        <Link href="/impressum" className="hover:text-bone">
          {t("Legal.impressum")}
        </Link>
        <Link href="/datenschutz" className="hover:text-bone">
          {t("Legal.datenschutz")}
        </Link>
      </div>
    </footer>
